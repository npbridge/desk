...
frappe.query_reports["Tickets Closed Over Time"] = {
  filters: [
    {
  ...
  get_chart_data: (_columns, result) => {
    ...
    const labels = largerDataSet.map(
      (data, index) =>
        `${firstDataset[index] ? firstDataset[index].modified : "-"}|
         ${secondDataset[index] ? secondDataset[index].modified : "-"}`
    );
        ...
        datasets: [
          {
            ...
            values: isFirstLarger
              ? firstDataSetValues
              : [...firstDataSetValues, ...arrayToFillSmallerDataset],
          },
          {
            name: "Second date range",
            values: isFirstLarger
              ? [...secondDatasetValues, ...arrayToFillSmallerDataset]
              : secondDatasetValues,
          },
        ],
      },
