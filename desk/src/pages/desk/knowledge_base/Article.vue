<template>
  <div class="pt-[17px] pl-[18px] pr-[24px]">
    <div class="flex flex-col md:flex-row md:space-x-[24px] h-full">
...
export default {
  ...
  mounted() {
    if (!this.articleId) {
      ...
      this.$event.emit('toggle_navbar_actions', {
        type: 'New Article',
        actions,
      })
    }
      ...
      if (!this.isNew) {
        ...
        if (doc) {
          const actions = [
            ...
            {
              label: doc.published ? 'Unpublish' : 'Publish',
              appearance: doc.published ? 'secondary' : 'primary',
              handler: () => {
                this.editMode = false
...

            {
              ...
              appearance: 'danger',
              handler: () => {
                this.$resources.deleteArticle.submit({
                  doctype: 'Article',
                  name: doc.name,
                })
              },
          ...
          this.$event.emit('toggle_navbar_actions', {
            type: doc.published ? 'Published Article' : 'Draft Article',
            actions,
          })
        }
      ...
      if (val) {
        const actions = [
          ...
          },
          {
            ...
            appearance: 'primary',
            handler: () => {
              this.saveArticleTitleAndContent()
            },
            dropdown: [
              {
                label: 'Publish',
                appearance: 'secondary',
                handler: () => {
                  this.saveArticleTitleAndContent(true)
                },
              },
            ],
          },
