...
export default {
  ...
  watch: {
    subCategory(val) {
      ...
      this.$event.emit('select_category', {
        name: this.subCategory,
        parent_category: this.category,
      })
    },
  },
  mounted() {
    const actions = [
      {
        ...
        handler: () => {
          this.$router.push({
            name: 'NewArticle',
            query: { category: this.subCategory },
          })
        },
        appearance: 'primary',
        dropdown: [
    ...
    this.$event.emit(
      ...
      this.category && this.subCategory
        ? { name: this.subCategory, parent_category: this.category }
        : null
    )
  },
  unmounted() {
  ...
  methods: {
    validateNewCategoryInputs() {
      ...
      if (!this.newCategoryInputValues.name) {
        ...
        this.newCategoryInputErrors.name =
