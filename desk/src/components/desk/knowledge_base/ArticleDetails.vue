    ...
    <div class="flex flex-col space-y-[12px] text-base">
      ...
      >
        <div>
          Published
          {{
            $dayjs.shortFormating($dayjs(article.published_on).fromNow())
          }}
          ago
        </div>
      ...
      <Input
        v-if="!$resources.users.loading"
        type="select"
        label="Author"
        :value="isNew ? user.user : article.author"
        @input="(val) => setArticleDetail('author', val)"
        :options="$resources.users.data.map((x) => x.name)"
      />
      <Input
        v-if="!$resources.categories.loading"
        type="select"
        label="Category"
        :value="isNew ? newArticleTempValues.category : article.category"
        @input="(val) => setArticleDetail('category', val)"
        :options="$resources.categories.data.map((x) => x.name)"
      />
      <Input
        type="textarea"
        label="Note"
