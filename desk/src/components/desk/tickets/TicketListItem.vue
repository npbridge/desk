<template>
  ...
  >
    ...
    >
      ...
      <div
        class="md:w-1/12 text-gray-600 font-normal hidden md:block"
        :style="
          ['Closed', 'Resolved'].includes(ticket.status) ? 'opacity: 0.5;' : ''
        "
      >
        {{ ticket.name }}
      </div>
      <div class="w-9/12 md:w-7/12">
        ...
        >
          <div
            class="lg:w-40 md:w-52 w-40 truncate"
            :class="
              !seen
          ...
          <div
            ...
            class="text-gray-600 font-medium bg-gray-200 px-[8px] py-[2px] rounded-[48px] uppercase text-xs hidden md:block"
          >
            {{ ticket.ticket_type }}
          </div>
        </router-link>
      ...
      >
        <div
          class="truncate w-40 text-gray-600 font-normal hidden md:block"
          v-if="ticket.contact"
        >
          {{ ticket.contact }}
        </div>
      ...
      <div
        class="md:w-2/12 font-normal hidden md:block"
        :style="
          ['Closed', 'Resolved'].includes(ticket.status) ? 'opacity: 0.5;' : ''
        "
      >
      ...
      <div class="pt-[-3px] md:w-1/12 ml-[2px] hidden md:block">
        <div>
