  ...
  <div v-if="ticket" class="flex flex-col h-full">
    ...
    >
      <div class="flex flex-row pb-[15px]">
        <div class="grow">
          <span class="text-[16px] font-normal text-gray-500">Ticket</span>
          <span class="text-[15px] font-semibold">{{ `#${ticket.name}` }}</span>
        </div>
          ...
          <div class="absolute w-[88.54px] flex flex-row">
            ...
            <div class="shrink-0">
              ...
              <div v-if="toggleStatuese">
                ...
                >
                  <div
                    v-for="status in ['Open', 'Replied', 'Resolved', 'Closed']"
                    :key="status"
                  >
                    <div
      ...
      <div class="text-base space-y-[11px]">
        <div class="flex flex-col space-y-[2px]">
          ...
          <div class="font-normal text-gray-900">
            {{ getFormatedDate(ticket.response_by, 'ddd, MMM DD, YYYY HH:mm') }}
          </div>
        </div>
        <div class="flex flex-col space-y-[2px]">
          <div class="flex flex-row space-x-[5.33px] items-center">
            ...
            <CustomIcons
              ...
              :name="
                { Success: 'sla-pass', Failed: 'sla-fail' }[resolutionStatus()]
              "
              class="w-[16px] h-[16px]"
            />
          ...
          <div class="font-normal text-gray-900">
            {{
              getFormatedDate(ticket.resolution_by, 'ddd, MMM DD, YYYY HH:mm')
            }}
          </div>
        </div>
      </div>
    ...
    <div class="px-[19px] py-[28px] h-full overflow-y-auto">
      <div class="text-base space-y-[12px]">
          ...
          >
            <template #input>
              ...
              >
                <div class="grow">
                  <div
                    v-if="ticket.assignees.length > 0 && !updatingAssignee"
                    class="flex flex-row text-left space-x-2"
                  >
                    <CustomAvatar
                      :label="ticket.assignees[0].agent_name"
                      :imageURL="ticket.assignees[0].image"
                      :imageOwner="ticket.assignees[0].name"
                      size="xs"
                    />
          ...
          >
            <template #input>
              <div class="flex flex-row space-x-1 items-center w-full">
                <div class="grow">
                  <div
                    v-if="ticket.ticket_type && !updatingTicketType"
                    class="text-left"
                  >
                    {{ ticket.ticket_type }}
                  </div>
        ...
        <div class="flex flex-col space-y-[8px]">
          ...
          <div
            v-if="ticket.ticket_tag.length > 0"
            ...
            <div v-for="tag in ticket.ticket_tag" :key="tag">
              ...
              >
                <div class="flex flex-row items-center h-[20px] space-x-[7px]">
                  <div class="text-[10px] uppercase grow">
                    {{
                      ticketTags.find((ttag) => ttag.name === tag.tag)
                        ?.description
                    }}
                  </div>
                  <div>
          ...
          <Autocomplete
            ...
            :value="
              ticket.ticket_tag.length > 0 && !updatingTicketTag
                ? ticket.ticket_tag[0].name
                : ''
            "
            @change="
              (item) => {
        ...
        <div class="flex flex-col space-y-[8px]">
          ...
          <Autocomplete
            ...
            :value="
              ticket.agent_group && !updatingTeam ? ticket.agent_group : ''
            "
            @change="
              (item) => {
                if (item.value) {
          ...
          >
            <template #input>
              <div class="flex flex-row space-x-1 items-center w-full">
                <div class="grow">
                  <div
                    v-if="ticket.agent_group && !updatingTeam"
                    class="text-left"
                  >
                    {{ ticket.agent_group }}
                  </div>
          ...
          >
            <template #input>
              <div class="flex flex-row space-x-1 items-center w-full">
                <div class="grow">
                  <div
                    v-if="ticket.priority && !updatingPriority"
                    class="text-left"
                  >
                    {{ ticket.priority }}
                  </div>
      ...
      <template #body-content>
        <div class="space-y-4">
          ...
          <div class="flex float-right space-x-2">
            ...
            >
            <Button @click="createTicketTypeFromDialog()" appearance="primary"
              >Create</Button
            >
          </div>
        </div>
    ...
    <Dialog
      :options="{ title: 'Create New Tag' }"
      v-model="openCreateNewTicketTagDialog"
    >
      <template #body-content>
        <div class="space-y-4">
          <Input type="text" v-model="newTag" placeholder="eg: Moodle" />
          <div class="flex float-right space-x-2">
            <Button @click="createAndAssignTicketTagFromDialog()"
              >Create and Assign</Button
            >
            <Button @click="createTicketTagFromDialog()" appearance="primary"
              >Create</Button
            >
          </div>
        </div>
  ...
  methods: {
    ...
    createAndAssignTicketTagFromDialog() {
      if (this.newTag) {
        ...
        this.ticketController
          .set(this.ticketId, 'tag', this.newTag)
          .then(() => {
